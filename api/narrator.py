...
def _narrate_patient(result: dict) -> str:
    ...
    # Overall risk (used to avoid contradictory "normal" phrasing)
    risk = result.get("risk_level", "NORMAL")

    # Conditions found
    conditions = result.get("detected_conditions", [])
    flags = result.get("abnormal_flags", [])
    # If no specialist-detected conditions but there are parameter flags,
    # reflect that in the opening sentence to avoid contradictory messaging.
    if not conditions or all("normal" in c.lower() for c in conditions):
        if flags:
            if risk == "HIGH":
                lines.append(
                    "No specialist-level conditions were detected, but some blood values are outside the normal range and your overall risk level is high."
                )
            else:
                lines.append("No specialist-level conditions were detected, but some blood values are outside the normal range.")
        else:
    ...
    # Short disclaimer
    lines.append("This report is AI-generated.")
